...
public abstract class TableProducerState extends ProducerState {
    ...
    /** Creates a state without filters or output schema (no-input fixtures). */
    ...
    /**
     * Creates a state pre-populating {@link #filters} and {@link #outputSchema}
     * from the init parameters, decoded once.
     *
     ...
     *
     * @param out the collector to emit a batch to or finish the stream on
     ...
     *
     * @param input the per-tick input batch carrying {@code custom_metadata}
     * @param out the collector to emit a batch to or finish the stream on
     * @param ctx the per-call context (logging, cancellation)
     */
