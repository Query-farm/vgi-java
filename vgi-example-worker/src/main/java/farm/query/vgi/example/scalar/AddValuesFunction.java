    ...
    public VectorSchemaRoot process(ScalarProcessParams params, VectorSchemaRoot input, BufferAllocator alloc) {
        ...
        if (TypeRules.isFloating(outType)) {
            ...
            for (int i = 0; i < rows; i++) {
                if (left.isNull(i) || right.isNull(i)) v.setNull(i);
                else v.setSafe(i, ScalarHelpers.toDouble(left, i) + ScalarHelpers.toDouble(right, i));
            }
        } else {
            BigIntVector v = (BigIntVector) out.getVector("result");
            for (int i = 0; i < rows; i++) {
                if (left.isNull(i) || right.isNull(i)) v.setNull(i);
                else v.setSafe(i, ScalarHelpers.toLong(left, i) + ScalarHelpers.toLong(right, i));
            }
        }
